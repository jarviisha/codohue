...
type ReembedWatcherRepo interface {
	...
	CountStaleCatalogItems(ctx context.Context, namespace, targetStrategyID, targetStrategyVersion string) (int, error)
	CountEmbeddedCatalogItems(ctx context.Context, namespace, targetStrategyID, targetStrategyVersion string) (int, error)
...
// SetLifecycleWriter fences completion writes against delete and reset.
func (w *ReembedWatcher) SetLifecycleWriter(writer interface {
	WithWriter(context.Context, string, func(context.Context, *nslifecycle.NamespaceLifecycle) error) error
}) {
	w.lifecycle = writer
}
...
func (w *ReembedWatcher) tick(ctx context.Context) {
	...
	for _, run := range runs {
		stale, err := w.repo.CountStaleCatalogItems(ctx, run.Namespace, run.TargetStrategyID, run.TargetStrategyVersion)
		if err != nil {
		...
		processed, err := w.repo.CountEmbeddedCatalogItems(ctx, run.Namespace, run.TargetStrategyID, run.TargetStrategyVersion)
		if err != nil {
		...
		complete := func(writeCtx context.Context) error {
			return w.repo.CompleteReembedRun(writeCtx, run.ID, processed, true, "", now, duration)
		}
		var completeErr error
		if w.lifecycle != nil {
			completeErr = w.lifecycle.WithWriter(ctx, run.Namespace, func(leased context.Context, _ *nslifecycle.NamespaceLifecycle) error {
				return complete(leased)
			})
		} else {
			completeErr = complete(ctx)
		}
		if completeErr != nil {
...
func (r *pgReembedRepo) ListOpenReembedRuns(ctx context.Context) ([]ReembedRun, error) {
	rows, err := r.db.Query(ctx, `
		SELECT id, namespace, COALESCE(target_strategy_id, ''), COALESCE(target_strategy_version, ''), started_at
		FROM batch_run_logs
	...
	for rows.Next() {
		var run ReembedRun
		if err := rows.Scan(&run.ID, &run.Namespace, &run.TargetStrategyID, &run.TargetStrategyVersion, &run.StartedAt); err != nil {
...
func (r *pgReembedRepo) CountStaleCatalogItems(ctx context.Context, namespace, targetStrategyID, targetStrategyVersion string) (int, error) {
	var n int
...
func (r *pgReembedRepo) CountEmbeddedCatalogItems(ctx context.Context, namespace, targetStrategyID, targetStrategyVersion string) (int, error) {
	var n int
	err := r.db.QueryRow(ctx, `
		SELECT COUNT(*)
		FROM catalog_items
