...
// Qdrant collections are addressed under the namespace's generation. Reading
// them under any other qualification serves a recreated namespace the
// previous incarnation's results — the exact failure the generation was
// introduced to prevent. (The trending ZSET key is derived inside
// internal/infra/redis from the raw namespace plus this same generation.)
func TestPhysicalNamespaces_MatchLifecycleResolver(t *testing.T) {
	for _, generation := range []int64{0, 1, 2, 17} {
		...
		if got := namespaceGeneration(cfg); got != want {
			t.Errorf("generation=%d resolved: got %d, want %d", generation, got, want)
		}
	...
	if got := namespaceGeneration(nil); got != 1 {
		t.Errorf("nil config generation: got %d, want 1", got)
	}
}
