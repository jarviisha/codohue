...
# Data-pump bot, shipped to a VPS by scp rather than as a published image.
# Override the target platform for non-x86 hosts, e.g. PUMP_GOARCH=arm64.
GEMINIPUMP_BIN := $(BIN_DIR)/geminipump
PUMP_GOOS      ?= linux
PUMP_GOARCH    ?= amd64
...
.PHONY: \
	build build-api build-cron build-admin build-admin-embed build-embedder build-geminipump \
	web-admin-deps web-admin-lint web-admin-test web-admin-build \
	run run-cron run-admin run-embedder run-loadgen run-geminipump dev dev-admin dev-embedder dev-all \
...
# Static binary for the Gemini data-pump bot. Deliberately outside `build` and
# outside every published image: the bot is an operator tool you copy to a host
# and run, not part of the deployed stack.
build-geminipump:
	CGO_ENABLED=0 GOOS=$(PUMP_GOOS) GOARCH=$(PUMP_GOARCH) \
		go build -ldflags="-s -w" -o $(GEMINIPUMP_BIN) ./examples/geminipump
...
# Example data-pump bot whose catalog content is generated by the Gemini API.
# Requires the full stack to be up and GEMINI_API_KEY set.
# Override knobs with ARGS, e.g. make run-geminipump ARGS="-rate 20 -max-items 500".
run-geminipump:
	go run ./examples/geminipump $(ARGS)
