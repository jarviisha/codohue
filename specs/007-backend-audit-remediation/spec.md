...
- Q: When consumers are unavailable long enough for Redis to reach its capacity, which guarantee takes priority? → A: Preserve every unprocessed entry and reject new publishes explicitly until capacity is restored; do not trim unprocessed backlog.
- Q: What lifecycle state applies when namespace deletion succeeds in some stores but fails in another? → A: Keep the namespace durably in `deleting`, return a retryable failure, and block writes and same-name recreation until cleanup completes successfully.
- Q: How is the trusted monitoring path protected? → A: Require a dedicated observability bearer token for metrics and detailed health diagnostics on the existing listener; keep the unauthenticated public health response sanitized.
- Q: How are legacy stream entries without a namespace generation handled during rollout? → A: Treat a missing generation as generation 1 only; once a namespace name has entered generation 2 or later, reject every entry without an explicitly matching generation.
- Q: What happens when identity-migration preflight cannot uniquely associate an existing vector point with a namespace and entity? → A: Stop the entire migration before any mutation, emit an actionable conflict report, and require operator repair before retrying.
...
5. **Given** namespace cleanup succeeds in some stores but fails in another, **When** the delete request returns, **Then** the namespace remains in `deleting`, all writes and same-name recreation are blocked, and retrying deletion resumes cleanup safely.
6. **Given** a legacy queued entry without a lifecycle generation, **When** it is consumed for a namespace that has never been recreated, **Then** it is treated as generation 1; for generation 2 or later, the same entry is rejected and cannot mutate state.
...
6. **Given** migration preflight finds a point whose namespace or entity identity is ambiguous, **When** migration is requested, **Then** no point or mapping is modified and an actionable conflict report identifies every unresolved point.
...
1. **Given** namespace configuration does not exist or its store is unavailable, **When** recommendation or ranking is requested, **Then** the request returns the stable 404 or 503 failure respectively, is not cached, and never uses defaults.
...
3. **Given** a valid dedicated observability bearer token, **When** health diagnostics and metrics are collected on the existing listener, **Then** all existing operational signals remain available.
4. **Given** a missing or invalid observability token, **When** a client requests metrics or detailed health diagnostics, **Then** the request is rejected without exposing protected content.
...
- Redis reaches its storage limit during a prolonged consumer outage; existing unprocessed entries remain recoverable while new publishes fail explicitly until capacity is restored.
- A namespace is deleted and recreated with the same name while old stream entries remain.
- A legacy stream entry has no generation field while the namespace is in generation 2 or later.
- Namespace cleanup partially succeeds before a dependency becomes unavailable; the durable `deleting` fence remains authoritative across process restarts.
...
- An existing vector point lacks sufficient authoritative metadata to resolve its namespace and entity type during migration preflight.
...
- **FR-002**: The system MUST bound completed stream history without deleting unprocessed or recoverable entries; when Redis capacity is exhausted, it MUST reject new publishes explicitly rather than trim unprocessed backlog.
- **FR-003**: Stream retention MUST cover behavioral event, catalog transport, and per-namespace embedding queues under a documented ownership and consumer-progress policy, including operational backpressure and recovery behavior at the storage limit.
- **FR-004**: Namespace deletion and application reset MUST establish a durable lifecycle boundary observed by every data-plane and background writer. A partially completed deletion MUST leave the namespace in `deleting`, reject writes and same-name recreation, return a retryable failure, and allow idempotent cleanup retries until success.
- **FR-005**: Work from an earlier namespace lifecycle MUST NOT mutate a later namespace created with the same name. A missing work-item generation MUST be interpreted as generation 1 only; generation 2 and later MUST require an explicit generation that matches the current namespace lifecycle.
...
- **FR-010**: The identity-mapping transition MUST include verified recovery for all existing vector producers, including producers that the server cannot recompute independently. Preflight MUST resolve each existing point to exactly one namespace and entity identity; otherwise it MUST stop before mutation and emit an actionable conflict report covering every unresolved point.
...
- **FR-019**: Operational metrics containing namespace labels MUST require a dedicated observability bearer token on the existing listener and MUST reject missing or invalid tokens without returning protected content.
- **FR-020**: Unauthenticated public health responses MUST expose only sanitized aggregate status, while detailed dependency diagnostics on the existing listener MUST require the dedicated observability bearer token.
...
- **Namespace Lifecycle**: One generation of a namespace name, with states that determine whether writes and background work are allowed. Generation numbers start at 1 and increase on same-name recreation. The `deleting` state is durable, rejects writes and same-name recreation, and persists until cross-store cleanup completes.
...
- **Vector Identity Mapping**: Association between namespace, entity type, string identifier, and numeric vector-store identifier; migration preflight must establish this association unambiguously before modifying any point or mapping.
- **Reconciliation Cursor**: Stable resume position that uniquely orders catalog changes.
- **Operational Endpoint**: Health or metric surface whose protected metrics and detailed diagnostics require a dedicated observability bearer token; its unauthenticated health response exposes only sanitized aggregate status.
...
- **SC-007**: A conflict-free migration rehearsal with cross-namespace and cross-entity identifier collisions preserves reachability of 100% of catalog, client-provided, sparse, and computed dense points; a rehearsal containing an unresolvable point modifies zero points or mappings and reports 100% of unresolved points.
...
- Preserving existing unprocessed work takes priority over accepting new publishes when Redis capacity is exhausted; producers receive an explicit retryable failure until capacity is restored.
- Operators can perform a staged rollout and maintenance window for identity migration when required.
- Trusted monitoring supplies a dedicated observability bearer token to the existing API listener; this mechanism does not require changes to the excluded web application.
