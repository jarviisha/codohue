...
| `id_mappings`              | String ID → BIGSERIAL numeric, **primary key `(namespace, entity_type, string_id)`**. Used as the Qdrant point ID to avoid hash collisions. One sequence serves every namespace and both entity types, and it is **capped at 2^32-1** (migration 030) because Qdrant sparse vectors index dimensions with `uint32`: the cap makes the ceiling a loud insert failure instead of silently unsearchable entities (§6). Resolution reads before it writes — the earlier `ON CONFLICT DO UPDATE` upsert burned a sequence value and wrote a dead tuple on every already-mapped id, which walked the ceiling ~80× faster than real growth. The batch form returns a **partial map**: an id whose insert conflicted and whose row then vanished is omitted rather than reported, so callers must not assume every requested id came back. Raising the ceiling needs a separate sparse index space, not a bigger cap |
...
```
score_final = alpha · clamp(score_sparse) + (1 - alpha) · bound(score_dense)
```

Normalization is **batch-independent** (one shared helper, `blendHybridScores`, used by both recommendations and rankings): sparse vectors are L2-normalized at compute time, so their dot products are cosines and only clamp to [0, 1]; dense cosine scores clamp the same way, and dot-distance namespaces (BYOE vectors of arbitrary magnitude) go through the fixed saturating curve `x/(x+k)` (`k`=5, a single global constant — a per-namespace or per-request value would reintroduce cross-request incomparability). This replaced per-request min-max, whose scores anchored to whatever else was in the batch, and later the saturating curve on sparse, whose fixed `k` flattened live scores (raw dots in the thousands all mapped to ~1.0, leaving freshness decay to decide the order). Sparse search runs against `{ns}_objects`, dense against `{ns}_objects_dense`; recommendations over-fetch 5×/3× the page, then each arm's candidates are **backfilled** on the other arm with a `HasID` query before blending — without that, a candidate outside one arm's top-K inherited 0 for that side ("not retrieved" read as "dissimilar") and a dense-only candidate capped at `(1-alpha)`. After backfill a missing score is a real verdict (not indexed on that side); an arm whose primary search failed is not backfilled, and backfill failure degrades to the zero-fill reading and marks the response degraded so it is not cached.

Because the clamp assumes unit-norm vectors, **a deploy that changes this scoring must be followed immediately by a full recompute** (`make run-cron`): until the new cron writes normalized vectors, stored raw dots all exceed 1 and clamp to exactly 1.0, collapsing sparse ordering onto freshness decay and the object-id tie-break. A namespace whose cron is stalled stays in that state and nothing alerts on it.
...
| GET    | `/v1/namespaces/{ns}/subjects/{id}/recommendations`  | CF recommendations (`?limit=&offset=`); per-item `scored` flag is false on the fallback sources, where `score` is a 0 placeholder |
...
| Batch-independent normalization (L2 at compute + clamp at serve; `x/(x+k)` only for dot-distance dense) | Per-request min-max anchored scores to the batch, so chunked rankings calls could not be merged; a fixed map keeps scores comparable across requests. Unit-norm vectors make sparse dots cosines, so magnitude (popularity) stops dominating and the serve-time map needs no calibration constant; `k` survives only where magnitudes are caller-controlled |
...
| Recommendations carry the same `scored` flag | `fallback_popular`, trending and `hybrid_cold` rank by a namespace-wide signal and emit `score: 0`, which read as "irrelevant to this subject" rather than "this response has no personalised score". The flag is per item, not a property of `source`: a `hybrid_cold` response that degraded to pass-through CF (trending and popular both unavailable) keeps the real scores those items were retrieved with |
| Route parameters decoded once, in `httpapi.URLParam` | chi matches on `RawPath`, so `did%3Aplc%3A…` reached handlers escaped and became a different subject — different id mapping, vectors, cache key and echoed `subject_id`. Decoding before routing is not an option: unescaping `%2F` would split one segment into two |
