...
- **Author is ownership metadata, never a behavioural link**: `catalog_items.author_subject_id` records which subject *created* an object. It shares the id space of `events.subject_id` but has no foreign key and is read by nothing in the recommendation path — not the embedder, not `cmd/cron`, not `recommend`. The subject↔object relation that drives CF lives solely in `events` and is **many-to-many**: the whole signal is that many subjects touch the same object. Giving an object one owning subject would not model that relation, it would destroy it. Author also never enters the content hash, so re-ingesting with only the author changed does not trigger a re-embed. Scope limit: it exists only on `catalog_items`, so only namespaces with `dense_source="catalog"` have anywhere to put it — under `item2vec`/`svd`/`byoe` an object has no row at all, only an id inside `events`.
