...
- **Dense source (unified)**: a single `dense_source` enum names the producer of object dense vectors — `disabled` | `item2vec` | `svd` | `byoe` | `catalog` (exported as `codohuetypes.DenseSource*` constants; **`catalog` is the core, recommended mode** — the only one where both dense collections have an in-system owner). It replaced the old `dense_strategy` + `catalog_enabled` pair (added in 016 with a dual-write window; the legacy columns were dropped in 017). `{ns}_subjects_dense` / `{ns}_objects_dense` hold the dense embeddings. When `alpha < 1.0` and `dense_source != "disabled"`, the recommend service blends sparse CF scores (weight=`alpha`) with dense scores (weight=`1-alpha`). Selecting `catalog` is how catalog auto-embedding is enabled — there is no separate boolean, so the old dense_strategy↔catalog conflict is structurally impossible.
- **Batch-independent score normalization**: the hybrid blend (shared by recommendations and rankings via one helper) maps sparse dot products through the fixed saturating curve `x/(x+k)` (`k`=5, a single global constant — deliberately not per-namespace or derived from per-request statistics, either of which would reintroduce cross-request incomparability) and bounds dense scores to [0,1] (cosine clamped; dot-distance namespaces saturate like sparse). This replaced per-request min-max, whose scores anchored to whatever else was in the batch — two chunks of one candidate list normalized independently and could not be merged. Consequence: rankings over chunked candidate sets are order-consistent with a single call over the union.
- **Rankings share Recommend's eligibility, unconditionally**: `Rank` merges the seen-items + `exclude_authored` exclusion set (same `excludedObjectIDs` path) as a `MustNot` on its candidate `HasID` filter, so one code path defines "eligible object" for both read surfaces. Excluded candidates return `scored:false` rather than being dropped, keeping the caller's list whole; exclusion-lookup failures degrade to unfiltered scoring, same as recommendations.
- **Silent dense downgrade is surfaced, not fixed silently**: a namespace configured for hybrid (`alpha<1`, dense on) whose `{ns}_subjects_dense` is empty serves sparse-only — under `byoe` this is the standing state until the client pushes subject vectors. The serving path logs it at warning level and the admin overview raises a per-namespace `dense_downgrade` alert; per-request visibility rides the `no_subject_vector` rank source.
...
| `POST`   | `/v1/namespaces/{ns}/catalog/batch`                                     | Batch catalog ingest (202): up to 100 items per request, validated independently with per-item results (`accepted`/`error` per item), so a repair walk costs O(corpus/100) requests. Namespace-level failures 404 like the single-item path |
| `GET`    | `/v1/namespaces/{ns}/catalog/objects`                                   | Reconciliation read (`?changed_since=RFC3339&limit=&offset=`): which object ids the namespace holds, ordered by `updated_at` ascending so a repair pass re-sends only the gap |
| `GET`    | `/v1/namespaces/{ns}/subjects/{id}/recommendations`                     | CF recommendations for a subject (`?limit=&offset=`)                   |
| `POST`   | `/v1/namespaces/{ns}/rankings`                                          | Score and rank up to 500 candidate items for a subject (200, computed). Hybrid: blends sparse CF + dense scores with the namespace `alpha` (dense-only when the subject has no sparse vector); applies the same seen-items + `exclude_authored` exclusions as recommendations. Every candidate comes back with a `scored` boolean — `false` means unscored (not indexed, or excluded), not "irrelevant"; a subject with no vector at all returns the whole list unscored with `source:"no_subject_vector"`. Scores are comparable across calls, so chunked requests merge into one ordering |
| `GET`    | `/v1/namespaces/{ns}/trending`                                          | Trending items from Redis ZSET (`?limit=&offset=`); `window_hours` in the response reports the namespace's configured window (a request param would be a lie — there is one ZSET per namespace) |
...
| `PUT`    | `/v1/namespaces/{ns}/objects/{id}/embedding`                            | Store/replace BYOE dense vector for an object (idempotent, 204). Optional `object_created_at` in the body feeds the γ-freshness rerank. Returns **409 Conflict** when the namespace has `dense_source="catalog"` (catalog auto-embedding is the source of truth in that mode). |
...
> **Ingest transport options:** Events can be sent via the HTTP endpoint above **or** published directly to Redis Streams — the `ingest` worker consumes both paths and writes to the same `events` table. The Redis Streams transport carries `namespace` inside the payload because it has no URL path. **Catalog content has the symmetric pair**: the `codohue:catalog` stream (JSON `CatalogStreamItem` in the `payload` field, no producer-side trim — entries persist until consumed and acked, so content published during an outage is ingested on recovery) is consumed by the same `cmd/api` worker process and feeds `catalog_items` + the embed pipeline exactly like HTTP ingest; validation is identical, and invalid stream items are observably rejected (warning log + `codohue_catalog_stream_rejects_total`), never silently dropped.

### REST API — `cmd/admin` (port 2002, session-cookie auth via `codohue_admin_session` **or** `Authorization: Bearer <CODOHUE_ADMIN_API_KEY>`)

Authentication models sessions as a resource for the browser UI (login = create session; logout = delete current session). Automation skips the cookie dance entirely: every `/api/admin/v1/*` route also accepts the global admin key as a bearer token. A bearer header, when present, is authoritative (the cookie is ignored — deterministic precedence), and failed bearer attempts consume the same per-IP failed-only budget as the login endpoint, so a correct key is never throttled while guessing is. `sdk/go/admin` wraps this path; `ProvisionCatalogNamespace` is the one-call provisioning road.
...
| `PUT`    | `/api/admin/v1/namespaces/{ns}`                                     | session       | Create/update namespace; calls `nsconfig.Service` directly (200 / 201). **PATCH semantics** — an omitted field leaves that column untouched; see the config-write note below. `dense_source="catalog"` is accepted here when `catalog_strategy_id` + `catalog_strategy_version` accompany it (same registry + dim validation as the catalog endpoint, one-request core-mode provisioning); without them it is rejected with an error naming the missing fields |
...
| `GET`    | `/api/admin/v1/catalog/strategies`                                  | session       | Embed strategy registry, namespace-free (`?dim=` narrows to strategies matching that `embedding_dim`; 503 when feature unwired). Exists because the create-namespace form needs the strategy picker *before* the namespace exists — the per-namespace catalog route 404s until then |
...
at specs/006-darkvoid-alignment/plan.md
<!-- SPECKIT END -->
