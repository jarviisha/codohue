...
- **Author is ownership metadata, never a behavioural link**: `catalog_items.author_subject_id` records which subject *created* an object. It shares the id space of `events.subject_id` but has no foreign key, and the only thing that ever reads it is the opt-in `exclude_authored` filter below — never the embedder, never `cmd/cron`, and never as a scoring signal. The subject↔object relation that drives CF lives solely in `events` and is **many-to-many**: the whole signal is that many subjects touch the same object. Giving an object one owning subject would not model that relation, it would destroy it. Author also never enters the content hash, so re-ingesting with only the author changed does not trigger a re-embed. Scope limit: it exists only on `catalog_items`. Nothing in the schema ties that table to `dense_source` (migration 010 declares no FK and no CHECK), so attributed rows *can* exist for any namespace — but the data-plane ingest is the only supported way to create them and it rejects anything other than `dense_source="catalog"`. In practice that makes `exclude_authored` a silent no-op elsewhere, which is why the namespace dashboard reports author coverage and the config page says so next to the toggle.
...
| `POST`   | `/v1/namespaces/{ns}/catalog`                                           | Ingest raw content for catalog auto-embedding (202; only when `dense_source="catalog"`; embedder consumer asynchronously upserts the dense vector). Optional `author_subject_id` records who created the object — ownership metadata only, see the design note below |
